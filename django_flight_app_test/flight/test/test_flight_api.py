...
#! UPDATE
    ...
    def test_flight_update_as_non_auth_user(self):
        data = {
            "flight_number": "456ewd",
            "operation_arirline": "THY",
            "departure_city": "Adana",
        ...
        print(self.flight.id)
        request = self.factory.put(
            '/flight/flights/1/', data)

        response = FlightView.as_view(
            {'put': 'update'})(request, pk='1')
        self.assertEqual(response.status_code, 401)

    def test_flight_update_as_auth_user(self):
        data = {
            "flight_number": "456ewd",
            "operation_arirline": "THY",
            "departure_city": "Adana",
...
#! DELETE
    ...
    def test_flight_delete_as_non_auth_user(self):
        request = self.factory.delete(
            '/flight/flights/')
        response = FlightView.as_view(
                     {'delete': 'destroy'})(request, pk='1')
        self.assertEqual(response.status_code, 401)
    ...
    def test_flight_delete_staff_user(self):
        request = self.factory.delete(
            '/flight/flights/', HTTP_AUTHORIZATION=f'Token {self.token}')
        self.user.is_staff = True
        self.user.save()
